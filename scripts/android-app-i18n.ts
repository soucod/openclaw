...
const ANDROID_SOURCE_ROOTS = [SOURCE_ROOT, WEAR_SOURCE_ROOT] as const;
...
async function readStrings(
  ...
  const source = await readFile(path.join(resourceRoot, locale, "strings.xml"), "utf8");
...
function localizeManualStrings(
  ...
  return [...base.values()].map((entry) => {
    ...
    if (translatable && !inventoryEntry) {
      throw new Error(`Wear string is missing from native inventory: ${JSON.stringify(source)}`);
    }
    const value =
...
export async function buildAndroidAppI18nCatalog(): Promise<GeneratedCatalog> {
  const [inventory, artifacts, localeStrings, wearBaseStrings, sourceFiles, toolDisplaySources] =
    await Promise.all([
      readInventory(),
      readArtifacts(),
...
export async function verifyAndroidAppI18n() {
  const [sourceFiles, base, referenceSource, wearBase, wearReferenceSource] = await Promise.all([
  ...
  process.stdout.write(
    `android-app-i18n: appSourceKeys=${baseKeys.size} wearSourceKeys=${wearBaseKeys.size}\n`,
  );
}
